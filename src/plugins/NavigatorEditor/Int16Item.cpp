...
Int16Item::Int16Item(char *newLabel,
		int16 newValue,
		uint32 level,
		bool expanded)
	: BaseListItem(B_INT16_TYPE, level, expanded)
{
	svalue			= new char[24];
	sprintf(svalue,"%d",(int)newValue);
...
void Int16Item::Update(BView *newOwner, const BFont *font)
{
...
void Int16Item::DrawItem(BView *owner, BRect bounds, bool complete)
{
...
void Int16Item::Select(void)
{
}
void Int16Item::Deselect(void)
{
}

void Int16Item::SetExpanded(bool /*expande*/)
{
};

status_t Int16Item::Invoke(BMessage *message)
{
	...
	if (sendMessage != NULL)
	{
		...
		valueContainer->AddInt32("type",B_INT16_TYPE);
		valueContainer->AddString("name",label);
		valueContainer->AddInt16("newValue", GetValue());
