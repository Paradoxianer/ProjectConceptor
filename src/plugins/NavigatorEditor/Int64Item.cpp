...
Int64Item::Int64Item(char *newLabel,
		int64 newValue,
		uint32 level,
		bool expanded)
	: BaseListItem(B_INT64_TYPE, level, expanded)
{
	svalue			= new char[24];
	sprintf(svalue,"%lld",(long long)newValue);
...
void Int64Item::Update(BView *newOwner, const BFont *font)
{
...
void Int64Item::DrawItem(BView *owner, BRect bounds, bool complete)
{
...
void Int64Item::Select(void)
{
}
void Int64Item::Deselect(void)
{
}

void Int64Item::SetExpanded(bool /*expande*/)
{
};

status_t Int64Item::Invoke(BMessage *message)
{
	...
	if (sendMessage != NULL)
	{
		...
		valueContainer->AddInt32("type",B_INT64_TYPE);
		valueContainer->AddString("name",label);
		valueContainer->AddInt64("newValue", GetValue());
