...
status_t MessageXmlWriter::WriteTo(BMessage &message, BPositionIO *destination){
	TiXmlDocument	xmlDoc;
	TiXmlDeclaration declare =  TiXmlDeclaration("1.0","UTF-8","yes");
	TiXmlComment	comment("Generated by MessageXmlWriter");
	xmlDoc.InsertEndChild(declare);
	xmlDoc.InsertEndChild(comment);
	xmlDoc.InsertEndChild(ProcessMessage(NULL,&message));
	std::string	xmlText;
	xmlText << xmlDoc;
	ssize_t written = destination->Write(xmlText.data(),xmlText.length());
	if ((written < 0) || ((size_t)written != xmlText.length()))
		return B_ERROR;
	return B_OK;
}
			...
			case B_POINTER_TYPE:{
				...
				for (q=0;q<count;q++) {
					...
					if (msg->FindPointer(name, q, &tmpPointer) == B_OK){
						// tmpPointer already IS the stored pointer value -
						// dereferencing it (as the previous code did) reads
						// whatever memory that address happens to point to,
						// which segfaults once the value is stale (e.g. a
						// pointer saved to disk in one process and reloaded
						// in another, as with any real ProjectConceptor
						// document's node/connection references).
						char *str = new char[64];
						sprintf(str, "%p", tmpPointer);
