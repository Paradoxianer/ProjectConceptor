...
Divider::Divider(BView *fV,BView *sV,orientation dr):BBox(BRect(0,0,0,0),"Divider",B_FOLLOW_ALL_SIDES,B_WILL_DRAW){
  direction=dr;
  firstView=fV;
  secondView=sV;
  move=false;
...
void Divider::SetFirstView(BView *fView){
 firstView=fView;
 if (direction==B_HORIZONTAL){
   ResizeTo(firstView->Frame().Width(),10);
   MoveTo(0,firstView->Frame().bottom+1);
...
void Divider::SetSecondView(BView *sView){
 secondView=sView;
  if (direction==B_HORIZONTAL){
    secondView->MoveTo(0,Frame().bottom+1);
    secondView->SetResizingMode(B_FOLLOW_LEFT_RIGHT);
...
void Divider::SetLocation(float location){
   if (location<GetMin()) location=GetMin();
   else if (location>GetMax()) location=GetMax();
   if (direction==B_HORIZONTAL){
...
void Divider::SetDirection(orientation dr){
...
void Divider::SetWidth(float width){
  if (direction==B_HORIZONTAL)
    ResizeTo(firstView->Bounds().Width(),width);
  else
    ResizeTo(width,firstView->Bounds().Height());
...
void Divider::MoveBy(float x, float y){
  if (x!=0)  MoveByX(x);
  if (y!=0)  MoveByY(y);
}


void Divider::MouseDown(BPoint point){
  uint32 buttons;
...
void Divider::MouseMoved(BPoint point, uint32 transit, const BMessage *message) {
