...
MessageListView::MessageListView(PDocument *document,BRect rect, BMessage * forContainer):BOutlineListView(rect,"MessageListView",B_SINGLE_SELECTION_LIST,B_FOLLOW_ALL_SIDES,B_WILL_DRAW | B_FRAME_EVENTS | B_NAVIGABLE| B_FULL_UPDATE_ON_RESIZE)
{
	doc				= document;
...
{
	container = message;
	baseEditMessage->ReplacePointer("node",container);
	delete editMessage;
	editMessage		= new BMessage(*baseEditMessage);
	ValueChanged();
}
